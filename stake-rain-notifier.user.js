...
// @version      1.5.113
// @description  StakePulse - Rain & Stats tracker pour Stake.bet - by alleluiateam | v1.5.113 - Ajout menu Wager Goals + Detecteur High Roller + Fix reset wager + Reset par periode + Session tracker + Migration wager goals + Fix Hot Words jour + Fix Emoji Stats jour + Filtre chats Hot Words + Fix Top Rainer jour + Fix Multi Tracker betId + notifs + Fix toggle + Fix seuil min 1.01
// @author       alleluiateam
...
(function () {
  ...
  function scanForBet(obj, parentObj) {
    ...
    if (wager.length > 2000) wager = wager.slice(-2000);
    save(SK_WAGER, wager);
...
'.srn-tick-down{color:#ff4444}',
  ...
  var CURRENT_VERSION = '1.5.113'; // Doit correspondre a @version
  ...
  function renderBetLog() {
    ...
    var rows = filtered.slice(0, 200).map(function(w) {
      ...
      var ps = p > 0 ? '+' : '';
      return '<div style="display:grid;grid-template-columns:80px 1fr 60px 60px;gap:4px;align-items:center;padding:5px 8px;border-bottom:1px solid #1e3a4a22;font-size:10px">'
    ...
    el.innerHTML = [
      ...
      '</div>',
      // En-tête tableau
      '<div style="display:grid;grid-template-columns:80px 1fr 60px 60px;gap:4px;padding:4px 8px;border-bottom:1px solid #1e3a4a;font-size:9px;color:#556677;font-weight:700">',
        '<span>DATE</span><span>JEU</span><span style="text-align:right">MISE</span><span style="text-align:right">PROFIT</span>',
      '</div>',
      // Lignes
      filtered.length === 0
