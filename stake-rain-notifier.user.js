...
(function () {
  ...
  var LANGS = {
    fr: {
      ...
      rains:            'Rains', settings:  'Config', converter: 'Convertisseur',
    },
    en: {
      ...
      rains:            'Rains', settings: 'Settings', converter: 'Converter',
    }
  };
...
'.srn-tick-down{color:#ff4444}',
  ...
  var CURRENT_VERSION = '1.5.64'; // Doit correspondre a @version
  ...
  function renderRainers() {
    ...
    var chatPref = load(SK_CHAT_PREF, '');
    var period = rainPeriod === 'week' ? 'week' : rainPeriod === 'month' ? 'month' : 'all';
    ...
    function renderSorted(sorted) {
      ...
      el.innerHTML = '<ul style="list-style:none;margin:0;padding:0">' + sorted.map(function(e, i) {
        var name = e.sender || e[0], usdTotal = e.usdTotal || (e[1] && e[1].totalUsd) || 0, count = e.count || (e[1] && e[1].count) || 0;
        var pos = medals[i] || (i+1) + '.';
        ...
        return '<li style="display:flex;align-items:center;gap:7px;padding:5px 0;border-bottom:1px solid #1e3a4a22;font-size:12px">'
          ...
          + amt + '<span style="color:#00d4ff;font-weight:700;font-size:11px;background:#00d4ff11;border-radius:6px;padding:2px 7px">' + count + ' rain' + (count > 1 ? 's' : '') + '</span></li>';
      }).join('') + '</ul>';
    }

    if (chatPref) {
      fetchLocalRainers(chatPref, period, function(err, rainers) {
        if (!err && rainers && rainers.length > 0) {
          renderSorted(rainers);
        } else {
          // Fallback local
          ...
          renderSorted(Object.values(senders).sort(function(a, b) { return b.usdTotal - a.usdTotal; }).slice(0, 10));
        }
      });
    } else {
      var log = load(SK_RAINERS, []);
      ...
      bridgeWS.addEventListener('message', function(e) {
        try {
          ...
          if (data.type === 'text' || data.type === 'mention') {
            ...
            console.log('[StakePulse] Bridge msg type=' + data.type + ' content=' + msgForEmoji.substring(0, 80));
            var emojiMatches = msgForEmoji.match(/:[a-z0-9_]+:/g);
            console.log('[StakePulse] Emoji matches:', emojiMatches);
            if (emojiMatches) {
              var msgChatId = data.chatId || '';
              var msgTs = data.ts || Date.now();
              // Structure : { blob: [{ts, chatId}, ...], pepe: [...] }
              var emojiLog = load(SK_EMOJI_COUNT, {});
              var changed = false;
              emojiMatches.forEach(function(m) {
                var name = m.replace(/:/g, '').toLowerCase();
                console.log('[StakePulse] Emoji "' + name + '" connu=' + STAKE_EMOJIS.includes(name));
                if (!STAKE_EMOJIS.includes(name)) return;
                if (!emojiLog[name]) emojiLog[name] = [];
                emojiLog[name].push({ ts: msgTs, chatId: msgChatId });
                // Garder max 5000 occurrences par emoji (évite de dépasser le storage)
                if (emojiLog[name].length > 5000) emojiLog[name] = emojiLog[name].slice(-5000);
                changed = true;
              });
              if (changed) { save(SK_EMOJI_COUNT, emojiLog); console.log('[StakePulse] Emoji sauvegardé:', emojiLog); }
            }
          }
