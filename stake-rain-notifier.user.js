...
// @version      1.7.6
// @description  StakePulse - Rain & Stats tracker pour Stake.bet - by alleluiateam | v1.7.6 - Ajout menu Wager Goals + Detecteur High Roller + Fix reset wager + Reset par periode + Session tracker + Migration wager goals + Fix Hot Words jour + Fix Emoji Stats jour + Filtre chats Hot Words + Fix Top Rainer jour + Fix Multi Tracker betId + notifs + Fix toggle + Fix seuil min 1.01
// @author       alleluiateam
...
'.srn-tick-down{color:#ff4444}',
  ...
  var CURRENT_VERSION = '1.7.6'; // Doit correspondre a @version
      ...
      bridgeWS.addEventListener('message', function(e) {
        try {
          ...
          if (data.type === 'bet' && data.bet) {
            // Ne pas traiter si on l'a envoyé nous-même (dédup bridge)
            var bId = data.bet.id || data.bet.nonce;
            if (bId && seenBetIds && seenBetIds.has(String(bId))) return;
