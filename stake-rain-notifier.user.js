...
// @version      1.5.129
// @description  StakePulse - Rain & Stats tracker pour Stake.bet - by alleluiateam | v1.5.129 - Ajout menu Wager Goals + Detecteur High Roller + Fix reset wager + Reset par periode + Session tracker + Migration wager goals + Fix Hot Words jour + Fix Emoji Stats jour + Filtre chats Hot Words + Fix Top Rainer jour + Fix Multi Tracker betId + notifs + Fix toggle + Fix seuil min 1.01
// @author       alleluiateam
...
'.srn-tick-down{color:#ff4444}',
  ...
  function refreshPanel() {
    ...
    var rains24h = logFiltered.filter(function(e) { return e.ts >= todayMidnight.getTime(); });
    ...
    if (badgesEl) {
      ...
      var todayRains = logFiltered.filter(function(e) { return e.ts >= dayStart2; });

      // 🍀 Chanceux : 3+ rains aujourd'hui
      if (todayRains.length >= 3) badges.push({ icon: '\uD83C\uDF40', label: 'Chanceux', color: '#00ff88', desc: todayRains.length + ' rains aujourd\'hui' });

      // 📅 Régulier : actif 7 jours consécutifs
      var streak7 = true;
      for (var d7 = 0; d7 < 7; d7++) {
        ...
        if (!logFiltered.some(function(e) { return e.ts >= dStart && e.ts < dEnd; })) { streak7 = false; break; }
      }
      if (streak7) badges.push({ icon: '\uD83D\uDCC5', label: 'Régulier', color: '#00d4ff', desc: '7 jours actifs de suite' });

      // 🏅 Fidèle : actif 30 jours consécutifs
      var streak30 = true;
      for (var d30 = 0; d30 < 30; d30++) {
        ...
        if (!logFiltered.some(function(e) { return e.ts >= d30Start && e.ts < d30End; })) { streak30 = false; break; }
      }
      if (streak30) badges.push({ icon: '\uD83C\uDFC5', label: 'Fidèle', color: '#ffd700', desc: '30 jours actifs de suite' });

      // 💰 Gros lot : rain 5$+ reçue
      var bigRain = logFiltered.some(function(e) { return parseFloat(e.usdEach || 0) >= 5; });
  ...
  var CURRENT_VERSION = '1.5.129'; // Doit correspondre a @version
