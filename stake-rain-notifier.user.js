...
// @version      1.5.105
// @description  StakePulse - Rain & Stats tracker pour Stake.bet - by alleluiateam | v1.5.105 - Ajout menu Wager Goals + Detecteur High Roller + Fix reset wager + Reset par periode + Session tracker + Migration wager goals + Fix Hot Words jour + Fix Emoji Stats jour + Filtre chats Hot Words + Fix Top Rainer jour + Fix Multi Tracker betId + notifs + Fix toggle + Fix seuil min 1.01
// @author       alleluiateam
...
(function () {
  ...
  function scanForBet(obj, parentObj) {
    ...
    // Deduplication — betId si disponible, sinon clé synthétique user+game+amount+fenetre 5s
    var betId = obj.id || obj.nonce || obj.betId || obj.hash || null;
...
'.srn-tick-down{color:#ff4444}',
  ...
  var CURRENT_VERSION = '1.5.105'; // Doit correspondre a @version
  ...
  function renderMultip() {
    ...
    var recentHtml = log.slice(0, 15).map(function(e) {
      ...
      var betIdHtml = e.betId ? '<a href="https://stake.bet/?betId=' + e.betId + '" target="_blank" style="color:#8899aa;font-size:9px;text-decoration:none;border:1px solid #1e3a4a;border-radius:4px;padding:1px 4px;white-space:nowrap" title="Voir le bet">🔗 ' + String(e.betId).slice(-6) + '</a>' : '';
      return '<div class="srn-multip-row srn-multip-hit" style="display:flex;align-items:center;gap:8px;padding:7px 10px;background:#162330;border:1px solid #1e3a4a;border-radius:8px;margin-bottom:5px">'
        + '<span class="srn-multip-val" style="color:' + color + ';font-size:15px;font-weight:800;min-width:64px">x' + e.multiplier.toFixed(2) + '</span>'
