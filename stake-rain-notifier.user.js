...
// @version      1.5.100
// @description  StakePulse - Rain & Stats tracker pour Stake.bet - by alleluiateam | v1.5.100 - Ajout menu Wager Goals + Detecteur High Roller + Fix reset wager + Reset par periode + Session tracker + Migration wager goals + Fix Hot Words jour + Fix Emoji Stats jour + Filtre chats Hot Words
// @author       alleluiateam
...
(function () {
  ...
  function scanForBet(obj, parentObj) {
    ...
    save(SK_WAGER, wager);
    // Session tracker + Migration wager goals + Fix Hot Words jour + Fix Emoji Stats jour + Filtre chats Hot Words — reset si jeu différent
...
'.srn-tick-down{color:#ff4444}',
  ...
  // Session tracker + Migration wager goals + Fix Hot Words jour + Fix Emoji Stats jour + Filtre chats Hot Words — reset automatique au changement de jeu
  ...
  var CURRENT_VERSION = '1.5.100'; // Doit correspondre a @version
  ...
  function renderRainers() {
    ...
    function buildFromLocal(log) {
      var now = Date.now();
      var startTs = rainPeriod === 'day' ? now - 24*3600*1000
