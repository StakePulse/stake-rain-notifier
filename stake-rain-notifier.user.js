...
'.srn-tick-down{color:#ff4444}',
  ...
  var CURRENT_VERSION = '1.5.61'; // Doit correspondre a @version
  ...
  function renderEmojiStats() {
    ...
    var chatPref = load(SK_CHAT_PREF, '');
    // Si "Mon chat" et qu'un chat est configuré, on charge les compteurs de ce chat
    var counts;
    if (filterVal === 'pref' && chatPref) {
      counts = load(SK_EMOJI_COUNT + '__' + chatPref, {});
    } else {
      counts = load(SK_EMOJI_COUNT, {});
    }
  ...
  window._srn = {
    ...
    resetEmojis: function() {
      var chatPref = load(SK_CHAT_PREF, '');
      save(SK_EMOJI_COUNT, {});
      if (chatPref) save(SK_EMOJI_COUNT + '__' + chatPref, {});
        ...
        try {
          ...
          if (data.type === 'text' || data.type === 'mention') {
            ...
            if (emojiMatches) {
              ...
              // Compteur par chatId : srn_emoji_count__chatId
              var countsByChat = msgChatId ? load(SK_EMOJI_COUNT + '__' + msgChatId, {}) : null;
              var changed = false;
              emojiMatches.forEach(function(m) {
                var name = m.replace(/:/g, '').toLowerCase();
                if (STAKE_EMOJIS.includes(name)) {
                  counts[name] = (counts[name] || 0) + 1;
                  if (countsByChat) countsByChat[name] = (countsByChat[name] || 0) + 1;
                  changed = true;
                }
              });
              if (changed) {
                save(SK_EMOJI_COUNT, counts);
                if (countsByChat && msgChatId) save(SK_EMOJI_COUNT + '__' + msgChatId, countsByChat);
              }
            }
          }
