...
'.srn-tick-down{color:#ff4444}',
  ...
  function renderMyStats() {
    ...
    var periodData = {
      day:     { rains: dayRains,   total: dayTotal,   label: "Aujourd'hui",  count: dayRains.length },
      week:    { rains: weekRains,  total: weekTotal,  label: 'Cette semaine', count: weekRains.length },
      month:   { rains: monthRains, total: monthTotal, label: 'Ce mois',       count: monthRains.length },
      allTime: { rains: myRains,    total: allTotal,   label: 'Tout temps',    count: myRains.length + offset.allTimeCount },
    };
    var pd = periodData[myStatsPeriod] || periodData.week;
    // Objectif semaine
    var WEEKLY_GOAL_USD = load('srn_weekly_goal', 300);
    var WEEKLY_GOAL_DYN = WEEKLY_GOAL_USD;
    if (curPrefM !== 'usd') {
      var upperCurG = curPrefM.toUpperCase();
      if (curPrefM === 'eur' && convRates['EUR'] && convRates['USD']) {
        WEEKLY_GOAL_DYN = WEEKLY_GOAL_USD * (convRates['EUR'] / convRates['USD']);
      } else if (convRates['USD'] && convRates[upperCurG]) {
        WEEKLY_GOAL_DYN = WEEKLY_GOAL_USD * (convRates[upperCurG] / convRates['USD']);
      }
    }
    ...
    Object.keys(weekMap).forEach(function(k) {
      if (weekMap[k].totalFiat > bestWeek) { bestWeek = weekMap[k].totalFiat; var d = weekMap[k].start; bestWeekLabel = d.getDate() + '/' + (d.getMonth()+1); }
    });
    // Streak
    var streak = 0;
    var checkWeek = getWeekStart(new Date()).getTime();
    for (var si = 0; si < 52; si++) {
      var wE = checkWeek + 7 * 24 * 3600 * 1000;
      var hasRain = myRains.some(function(e) { return e.ts >= checkWeek && e.ts < wE; });
      if (!hasRain) break;
      streak++;
      checkWeek -= 7 * 24 * 3600 * 1000;
    }
    // Moy. par rain
    ...
    for (var wi = 7; wi >= 0; wi--) {
      ...
      var wD = new Date(wS);
      weeks.push({ total: wT, label: wD.getDate() + '/' + (wD.getMonth()+1), isCurrent: isCurrent, count: wRains.length });
    }
    ...
    var barsHtml = weeks.map(function(w) {
      ...
      return '<div style="display:flex;flex-direction:column;align-items:center;flex:1;gap:2px">'
        ...
        + '<div style="width:100%;background:' + barColor + ';border-radius:3px 3px 0 0;height:' + h + 'px;border:' + border + '" title="' + w.total.toFixed(2) + symM + ' - ' + w.count + ' rain(s)"></div></div>';
    }).join('');
    ...
    el.innerHTML = [
      ...
      '<div class="srn-stat-card" style="margin-bottom:8px">',
        '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">',
          '<span style="font-size:11px;color:#8899aa">🎯 Objectif semaine</span>',
          '<span style="font-size:12px;font-weight:700;color:' + pctColor + '">' + (goalReached ? '🎉 ' : '') + pct + '%</span>',
        '</div>',
        ...
        '<div style="display:flex;justify-content:space-between;font-size:10px;color:#8899aa">',
          '<span>' + weekTotal.toFixed(2) + symM + ' / ' + parseFloat(WEEKLY_GOAL_DYN).toFixed(2) + symM + '</span>',
          '<span>Reste : ' + Math.max(0, WEEKLY_GOAL_DYN - weekTotal).toFixed(2) + symM + '</span>',
        '</div>',
      '</div>',
      // Stats secondaires
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:8px">',
        ...
        '<div class="srn-stat-card"><div class="srn-stat-card-title">🔥 Streak</div><div class="srn-stat-card-val" style="color:' + (streak >= 3 ? '#ff9500' : streak >= 1 ? '#ffd700' : '#8899aa') + '">' + streak + ' sem.</div><div class="srn-stat-card-sub">' + (streak >= 3 ? 'En feu !' : streak >= 1 ? 'Continue !' : 'Aucun streak') + '</div></div>',
  ...
  var CURRENT_VERSION = '1.5.41'; // Doit correspondre a @version
