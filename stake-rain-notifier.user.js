...
// @version      1.5.98
// @description  StakePulse - Rain & Stats tracker pour Stake.bet - by alleluiateam | v1.5.98 - Ajout menu Wager Goals + Detecteur High Roller + Fix reset wager + Reset par periode + Session tracker + Migration wager goals + Fix Hot Words jour
// @author       alleluiateam
...
(function () {
  ...
  function scanForBet(obj, parentObj) {
    ...
    save(SK_WAGER, wager);
    // Session tracker + Migration wager goals + Fix Hot Words jour — reset si jeu différent
...
'.srn-tick-down{color:#ff4444}',
  ...
  // Session tracker + Migration wager goals + Fix Hot Words jour — reset automatique au changement de jeu
  ...
  var CURRENT_VERSION = '1.5.98'; // Doit correspondre a @version
  ...
  function renderEmojiStats() {
    ...
    var now = Date.now();
    var since = 0;
    if (wordPeriod === 'day')   since = now - 24 * 60 * 60 * 1000;
