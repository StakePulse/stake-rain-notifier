...
// @version      1.5.90
// @description  StakePulse - Rain & Stats tracker pour Stake.bet - by alleluiateam | v1.5.90 - Ajout menu Wager Goals
// @author       alleluiateam
...
'.srn-tick-down{color:#ff4444}',
  ...
  var CURRENT_VERSION = '1.5.90'; // Doit correspondre a @version
  ...
  function renderWagerGoals() {
    ...
    var goals = [
      { label: '🌧 Éligibilité Rain',               target: 3500  },
    ...
    var rows = goals.map(function(g) {
      ...
      var badge  = done ? '<span style="color:#00ff88;font-size:10px;font-weight:700;margin-left:4px">✓ ATTEINT</span>' : '';
      return [
        '<div style="margin-bottom:14px">',
          '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">',
            '<span style="font-size:11px;color:#cdd9e5;font-weight:600">' + g.label + badge + '</span>',
            '<span style="font-size:11px;color:#8899aa">' + fmtUsd(total7d) + ' / ' + fmtUsd(g.target) + '</span>',
          '</div>',
          '<div style="background:#0d1f2d;border-radius:6px;height:10px;overflow:hidden">',
            '<div style="height:100%;width:' + pct.toFixed(1) + '%;background:' + color + ';border-radius:6px;transition:width .4s"></div>',
          '</div>',
          (!done ? '<div style="font-size:10px;color:#556677;margin-top:3px">encore ' + fmtUsd(remain) + ' à wager</div>' : ''),
        '</div>',
      ].join('');
    }).join('');

    var lastUpdate = wager.length > 0
      ? '<div style="font-size:10px;color:#445566;margin-top:8px;text-align:right">Dernier bet : '
        + new Date(wager[wager.length-1].ts).toLocaleTimeString('fr-FR', {hour:'2-digit',minute:'2-digit'})
        + '</div>'
      : '';

    el.innerHTML = [
      '<div style="font-size:12px;color:#8899aa;margin-bottom:14px;line-height:1.4">',
        'Wager des <b style="color:#00d4ff">7 derniers jours glissants</b><br>',
        '<span style="font-size:13px;color:#fff;font-weight:700">Total : ' + fmtUsd(total7d) + '</span>',
      '</div>',
      rows,
      lastUpdate,
    ].join('');
