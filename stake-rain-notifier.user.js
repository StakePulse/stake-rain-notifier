    ...
    fbGet('shared', function(data) {
      ...
      FIREBASE_KEYS.forEach(function(key) {
        if (data[key] !== undefined && data[key] !== null) {
          // Si un reset a eu lieu après la dernière sync, on ignore Firebase pour cette clé
          var lastReset = resetTs[key] || 0;
          ...
          if (Array.isArray(data[key]) && Array.isArray(local)) {
            ...
            // Si reset récent : on ignore Firebase et on garde le local vide
            if (lastReset > 0) {
              // On filtre les entrées Firebase antérieures au reset
              var filtered = {};
              Object.keys(data[key]).forEach(function(k) {
                // data[key][k] est un compteur (number), pas un timestamp — on ignore tout
              });
              // Merge uniquement le local (post-reset) par-dessus le filtered vide
              var merged2 = Object.assign({}, filtered, local || {});
              save(key, merged2);
            } else {
            ...
            if (!lastReset) save(key, data[key]);
          }
        }
      });
...
'.srn-tick-down{color:#ff4444}',
  ...
  var CURRENT_VERSION = '1.5.59'; // Doit correspondre a @version
  ...
  window._srn = {
    ...
    resetEmojis: function() {
      save(SK_EMOJI_COUNT, {});
      fbSet('shared/srn_emoji_count', {});
      // Marque le timestamp de reset pour bloquer le fbPull
      var resetTs = load(SK_RESET_TS, {});
      resetTs[SK_EMOJI_COUNT] = Date.now();
