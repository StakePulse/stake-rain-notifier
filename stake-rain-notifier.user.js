...
// @version      1.5.97
// @description  StakePulse - Rain & Stats tracker pour Stake.bet - by alleluiateam | v1.5.97 - Ajout menu Wager Goals + Detecteur High Roller + Fix reset wager + Reset par periode + Session tracker + Migration wager goals
// @author       alleluiateam
...
(function () {
  ...
  function scanForBet(obj, parentObj) {
    ...
    save(SK_WAGER, wager);
    // Session tracker + Migration wager goals — reset si jeu différent
...
'.srn-tick-down{color:#ff4444}',
  ...
  // Session tracker + Migration wager goals — reset automatique au changement de jeu
  ...
  function renderModeration() {
    ...
    var now = Date.now();
    var counts = load(SK_WORDCOUNT, []);
    var periodMs = wordPeriod === 'alltime' ? Infinity : ({ day: 24*3600*1000, week: 7*24*3600*1000, month: 30*24*3600*1000 }[wordPeriod] || 24*3600*1000);
    var filtered = periodMs === Infinity ? counts : counts.filter(function(c) { return now - c.ts <= periodMs; });
    var totals = {};
  ...
  var CURRENT_VERSION = '1.5.97'; // Doit correspondre a @version
