...
// @version      1.5.109
// @description  StakePulse - Rain & Stats tracker pour Stake.bet - by alleluiateam | v1.5.109 - Ajout menu Wager Goals + Detecteur High Roller + Fix reset wager + Reset par periode + Session tracker + Migration wager goals + Fix Hot Words jour + Fix Emoji Stats jour + Filtre chats Hot Words + Fix Top Rainer jour + Fix Multi Tracker betId + notifs + Fix toggle + Fix seuil min 1.01
// @author       alleluiateam
...
(function () {
  ...
  function scanForBet(obj, parentObj) {
    ...
    // Deduplication — nonce = betId public Stake (ex: 477468706059), sinon UUID interne, sinon clé synthétique
    var betId = obj.nonce || obj.betId || obj.id || obj.hash || null;
    ...
    // DEBUG temporaire — loggue tous les champs de l'objet pour trouver le bon betId
    if (MULTIP_DEBUG) {
      console.log('[StakePulse][BETID DEBUG] id=' + obj.id + ' nonce=' + obj.nonce + ' betId=' + obj.betId + ' hash=' + obj.hash);
      console.log('[StakePulse][BETID DEBUG] all keys:', Object.keys(obj).join(', '));
      console.log('[StakePulse][BETID DEBUG] full obj:', JSON.stringify(obj).substring(0, 500));
      if (parentObj) console.log('[StakePulse][BETID DEBUG] parentObj keys:', Object.keys(parentObj).join(', '), '| parentObj:', JSON.stringify(parentObj).substring(0, 300));
    }
...
'.srn-tick-down{color:#ff4444}',
  ...
  function buildSetup() {
    ...
    var MENU_LABELS = {
      ...
      messages:'Messages', mystats:'Statistiques de rains', moderation:'Hot Words',
      rainers:'Top Rainer', wager:'Statistiques en direct', wagergoals:'\uD83C\uDFAF Wager Goals', multip:'Multi Tracker',
  ...
  var CURRENT_VERSION = '1.5.109'; // Doit correspondre a @version
