...
(function () {
  ...
  var FIREBASE_KEYS = ['srn_rankings', 'srn_wordcount', 'srn_emoji_count', 'srn_multip', 'srn_wager'];
        ...
        if (data[key] !== undefined && data[key] !== null) {
          ...
          if (Array.isArray(data[key]) && Array.isArray(local)) {
            ...
            data[key].forEach(function(e) {
              ...
              // Skip si déjà reçu via bridge (même sender+chatId dans la même minute)
  ...
  var SK_EMOJI_URLS = 'srn_emoji_urls'; // cache des vraies URLs Stake (avec hash)
  function trackEmojis(node) {
    ...
    var emojiUrls = load(SK_EMOJI_URLS, {});
    var changed = false;
    emojiImgs.forEach(function(img) {
      ...
      changed = true;
      // Mémorise la vraie URL (avec hash Vite) depuis le DOM de Stake
      if (img.src && img.src.indexOf('stake') >= 0 && !emojiUrls[alt]) {
        emojiUrls[alt] = img.src;
        console.log('[StakePulse] URL emoji cachee:', alt, img.src);
      }
      console.log('[StakePulse] Emoji detecte:', alt);
    });
    if (changed) {
      save(SK_EMOJI_COUNT, counts);
      save(SK_EMOJI_URLS, emojiUrls);
    }
  }
...
'.srn-tick-down{color:#ff4444}',
  ...
  var CURRENT_VERSION = '1.5.55'; // Doit correspondre a @version
  ...
  function renderEmojiStats() {
    ...
    if (!el) return;
    el.innerHTML = '<div class="srn-empty" style="font-size:11px">Chargement\u2026</div>';
    var fbUrl = 'https://stakepulse-v2.europe-west1.firebasedatabase.app/shared/srn_emoji_count.json';
    ...
    GM_xmlhttpRequest({
      ...
      onload: function(res) {
        try {
          var counts = JSON.parse(res.responseText) || {};
          ...
          var max = sorted[0][1];
          var gifEmojis = ['blob','nyancat','coin','catbread'];
          var emojiUrls = load(SK_EMOJI_URLS, {});
          el.innerHTML = sorted.map(function(e, idx) {
            var name = e[0], count = e[1];
            var pct  = Math.round((count / max) * 100);
            var ext  = gifEmojis.indexOf(name) >= 0 ? 'gif' : 'png';
            // Priorité 1 : URL réelle capturée depuis le DOM (avec hash Vite)
            // Priorité 2 : performance.getEntriesByType (assets déjà chargés)
            // Priorité 3 : URL générique fallback
            var cachedUrl = emojiUrls[name];
            if (!cachedUrl) {
              var entries = performance.getEntriesByType('resource');
              var found = entries.find(function(r) {
                return r.name.indexOf('/' + name + '.') >= 0 && (r.name.indexOf('.gif') >= 0 || r.name.indexOf('.png') >= 0 || r.name.indexOf('.webp') >= 0);
              });
              if (found) cachedUrl = found.name;
            }
            var imgSrc = cachedUrl || ('https://stake.bet/_app/immutable/assets/' + name + '.' + ext);
            ...
            return '<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;background:#162330;border:1px solid #1e3a4a;border-radius:8px;padding:6px 10px">'
              ...
              + '<img src="' + imgSrc + '" style="width:28px;height:28px;object-fit:contain;flex-shrink:0"/>'
        ...
        } catch(err) { el.innerHTML = '<div class="srn-empty">Erreur de chargement.</div>'; }
      },
      onerror: function() { el.innerHTML = '<div class="srn-empty">Erreur r\u00e9seau.</div>'; }
    });
  }
