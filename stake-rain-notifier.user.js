...
// @version      1.7.10
// @description  StakePulse - Rain & Stats tracker pour Stake.bet - by alleluiateam | v1.7.10 - Ajout menu Wager Goals + Detecteur High Roller + Fix reset wager + Reset par periode + Session tracker + Migration wager goals + Fix Hot Words jour + Fix Emoji Stats jour + Filtre chats Hot Words + Fix Top Rainer jour + Fix Multi Tracker betId + notifs + Fix toggle + Fix seuil min 1.01
// @author       alleluiateam
...
(function () {
  ...
  function fbPush() {
    ...
    FIREBASE_KEYS.forEach(function(key) {
      ...
      // Pour srn_rain_log : limiter aux 30 derniers jours pour Firebase
      if (key === 'srn_rain_log' && Array.isArray(val)) {
        var since30d = now - 30 * 24 * 3600 * 1000;
        val = val.filter(function(e) { return e.ts >= since30d; }).slice(-500);
      }
...
'.srn-tick-down{color:#ff4444}',
  ...
  var CURRENT_VERSION = '1.7.10'; // Doit correspondre a @version
