...
// @version      1.5.114
// @description  StakePulse - Rain & Stats tracker pour Stake.bet - by alleluiateam | v1.5.114 - Ajout menu Wager Goals + Detecteur High Roller + Fix reset wager + Reset par periode + Session tracker + Migration wager goals + Fix Hot Words jour + Fix Emoji Stats jour + Filtre chats Hot Words + Fix Top Rainer jour + Fix Multi Tracker betId + notifs + Fix toggle + Fix seuil min 1.01
// @author       alleluiateam
...
'.srn-tick-down{color:#ff4444}',
  ...
  var CURRENT_VERSION = '1.5.114'; // Doit correspondre a @version
  ...
  function renderBetLog() {
    ...
    // Export CSV
    function exportCSV() {
      var rows = ['Date,Jeu,Mise,Payout,Profit,Devise'];
      ...
      a.href = url; a.download = 'stakepulse_bets_' + new Date().toISOString().slice(0,10) + '.csv';
