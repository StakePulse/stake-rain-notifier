...
// @version      1.7.7
// @description  StakePulse - Rain & Stats tracker pour Stake.bet - by alleluiateam | v1.7.7 - Ajout menu Wager Goals + Detecteur High Roller + Fix reset wager + Reset par periode + Session tracker + Migration wager goals + Fix Hot Words jour + Fix Emoji Stats jour + Filtre chats Hot Words + Fix Top Rainer jour + Fix Multi Tracker betId + notifs + Fix toggle + Fix seuil min 1.01
// @author       alleluiateam
    ...
    window.addEventListener("srn_fetch_message", function(e) {
      try {
        ...
        if (betData && betData.amount !== undefined) {
          ...
          if (fetchBetId) seenFetchBetIds.add(String(fetchBetId));
          var betPayload = {
...
'.srn-tick-down{color:#ff4444}',
  ...
  var CURRENT_VERSION = '1.7.7'; // Doit correspondre a @version
