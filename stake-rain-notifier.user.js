...
// @version      1.5.119
// @description  StakePulse - Rain & Stats tracker pour Stake.bet - by alleluiateam | v1.5.119 - Ajout menu Wager Goals + Detecteur High Roller + Fix reset wager + Reset par periode + Session tracker + Migration wager goals + Fix Hot Words jour + Fix Emoji Stats jour + Filtre chats Hot Words + Fix Top Rainer jour + Fix Multi Tracker betId + notifs + Fix toggle + Fix seuil min 1.01
// @author       alleluiateam
...
'.srn-tick-down{color:#ff4444}',
  ...
  var CURRENT_VERSION = '1.5.119'; // Doit correspondre a @version
  ...
  // Chargement dynamique de SheetJS via GM_xmlhttpRequest (contourne la CSP)
  var XLSX = null;
    ...
    GM_xmlhttpRequest({
      ...
      onload: function(res) {
        try {
          ...
          // eval dans le scope Tampermonkey — hors de la CSP Stake
          (function(){ eval(res.responseText); XLSX = window.XLSX; }).call(window);
          if (!XLSX) {
            // Fallback : exécuter dans scope local
            eval(res.responseText);
          }
          console.log('[StakePulse] SheetJS chargé :', typeof XLSX);
