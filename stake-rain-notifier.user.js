...
// @version      1.5.101
// @description  StakePulse - Rain & Stats tracker pour Stake.bet - by alleluiateam | v1.5.101 - Ajout menu Wager Goals + Detecteur High Roller + Fix reset wager + Reset par periode + Session tracker + Migration wager goals + Fix Hot Words jour + Fix Emoji Stats jour + Filtre chats Hot Words + Fix Top Rainer jour
// @author       alleluiateam
...
(function () {
  ...
  function scanForBet(obj, parentObj) {
    ...
    // Deduplication stricte — on exige un betId unique
    var betId = obj.id || obj.nonce || obj.betId || null;
    if (!betId) return; // Sans ID on ne peut pas dédupliquer fiablement
    var dedupKey = String(betId);
    ...
    save(SK_WAGER, wager);
    // Session tracker + Migration wager goals + Fix Hot Words jour + Fix Emoji Stats jour + Filtre chats Hot Words + Fix Top Rainer jour — reset si jeu différent
    ...
    if (amount > 0 && payout > 0) {
      ...
      if (cfg.enabled) {
        ...
        if (gameThreshold !== null && mult >= gameThreshold) {
          checkMultiplier(game, mult, betId);
        }
      }
    }
...
'.srn-tick-down{color:#ff4444}',
  ...
  // Session tracker + Migration wager goals + Fix Hot Words jour + Fix Emoji Stats jour + Filtre chats Hot Words + Fix Top Rainer jour — reset automatique au changement de jeu
  ...
  var CURRENT_VERSION = '1.5.101'; // Doit correspondre a @version
