...
// @version      1.5.123
// @description  StakePulse - Rain & Stats tracker pour Stake.bet - by alleluiateam | v1.5.123 - Ajout menu Wager Goals + Detecteur High Roller + Fix reset wager + Reset par periode + Session tracker + Migration wager goals + Fix Hot Words jour + Fix Emoji Stats jour + Filtre chats Hot Words + Fix Top Rainer jour + Fix Multi Tracker betId + notifs + Fix toggle + Fix seuil min 1.01
// @author       alleluiateam
...
(function () {
  ...
  function isHighRollerNode(node) {
    ...
    // Cherche dans le noeud ET ses parents proches (jusqu'à 10 niveaux)
    var el = node;
    for (var i = 0; i < 10 && el; i++) {
      var html = el.innerHTML || '';
      // Badge VIP Stake : SVG avec ID contenant "VIP" (ex: VIPPlatinumIV__a)
      ...
      if (el.querySelector) {
        ...
        if (hrEl) return true;
        // linearGradient ou SVG avec ID VIP
        var defs = el.querySelectorAll('defs linearGradient[id], svg [id]');
        for (var d = 0; d < defs.length; d++) {
          if (/VIP/i.test(defs[d].getAttribute('id') || '')) return true;
        }
      }
      el = el.parentElement;
    }
...
'.srn-tick-down{color:#ff4444}',
  ...
  var CURRENT_VERSION = '1.5.123'; // Doit correspondre a @version
