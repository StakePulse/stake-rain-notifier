...
// @version      1.7.12
// @description  StakePulse - Rain & Stats tracker pour Stake.bet - by alleluiateam | v1.7.12 - Ajout menu Wager Goals + Detecteur High Roller + Fix reset wager + Reset par periode + Session tracker + Migration wager goals + Fix Hot Words jour + Fix Emoji Stats jour + Filtre chats Hot Words + Fix Top Rainer jour + Fix Multi Tracker betId + notifs + Fix toggle + Fix seuil min 1.01
// @author       alleluiateam
  ...
  function interceptWebSocket() {
    ...
    window.addEventListener("srn_ws_message", function(e) {
      try {
        var d = e.detail;
        scanPayload(d);
        // Détection High Roller via isHighroller: true
        var payload = d && d.payload && d.payload.data && d.payload.data.chatMessages;
        if (payload && payload.user && payload.user.isHighroller && payload.data && payload.data.message) {
          var hrMsg = {
            type:     'highroller',
          ...
          if (typeof bridgeWS !== 'undefined' && bridgeWS && bridgeWS.readyState === 1) {
            bridgeWS.send(JSON.stringify(hrMsg));
          }
        }
      } catch(err) {}
    });
...
'.srn-tick-down{color:#ff4444}',
  ...
  var CURRENT_VERSION = '1.7.12'; // Doit correspondre a @version
