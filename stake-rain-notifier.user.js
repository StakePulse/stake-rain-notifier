...
// @version      1.6.5
// @description  StakePulse - Rain & Stats tracker pour Stake.bet - by alleluiateam | v1.6.5 - Wager Goals, High Roller detector, Multi Tracker fixes, Journal des bets, Badges & Disette, Export Excel
// @author       alleluiateam
...
(function () {
  ...
  function isHighRollerNode(node) {
    ...
    function hasVIPBadge(el) {
      ...
      return html.indexOf('fed43f') >= 0 ||
             html.indexOf('VIPPlatinum') >= 0 || html.indexOf('VIPDiamond') >= 0 ||
             html.indexOf('VIPGold') >= 0 || html.indexOf('VIPSilver') >= 0 ||
             html.indexOf('VIPBronze') >= 0 || html.indexOf('VIPObsidian') >= 0;
    }
    // Vérifier le noeud + parents + siblings à chaque niveau
    var el = node;
    for (var i = 0; i < 15 && el; i++) {
      if (hasVIPBadge(el)) return true;
      // Vérifier les frères (siblings) du noeud courant
      if (el.parentElement) {
  ...
  function checkHighRollerBet(node, text, sender) {
    ...
    if (!sender) return;
    // Le joueur doit avoir le badge VIP (flamme) dans son noeud DOM
    ...
    console.log('[StakePulse] \uD83D\uDD25 High Roller detecte :', sender, trimmed);
  }
...
'.srn-tick-down{color:#ff4444}',
  ...
  var CURRENT_VERSION = '1.6.5'; // Doit correspondre a @version
