...
'.srn-tick-down{color:#ff4444}',
    ...
    panelEl.innerHTML = [
      ...
      '<div id="srn-body">',
        ...
        '<div class="srn-sec active" id="tab-dashboard">',
          ...
          '<div class="srn-row"><span class="srn-lbl">Chat surveillé</span><span class="srn-val" id="srn-chat-label" style="color:#00d4ff;font-size:11px">-</span></div>',
    ...
    if (chatPref) {
      fetchLocalStats(chatPref, function(err, stats) {
        if (!err && stats) {
          ...
          if (g('srn-lr') && stats.lastRain) g('srn-lr').textContent = new Date(stats.lastRain).toLocaleString('fr-FR', { day:'2-digit', month:'2-digit', hour:'2-digit', minute:'2-digit' });
        } else {
  ...
  function renderMyStats() {
    ...
    // Streak — rains consécutives (pas de rain manquée entre les miennes)
    var sortedRains = myRains.slice().sort(function(a,b) { return b.ts - a.ts; });
    var streak = 0;
    if (sortedRains.length > 0) {
      streak = 1;
      for (var si = 1; si < sortedRains.length; si++) {
        // Chercher si entre la rain[si-1] et rain[si] il y a eu une rain sans moi
        var tAfter  = sortedRains[si-1].ts;
        var tBefore = sortedRains[si].ts;
        var rainsBetween = log.filter(function(e) {
          return e.ts > tBefore && e.ts < tAfter;
        });
        var missedOne = rainsBetween.some(function(e) {
          return !e.recipients || !e.recipients.some(function(r) { return r.toLowerCase() === me; });
        });
        if (missedOne) break;
        streak++;
  ...
  var CURRENT_VERSION = '1.5.44'; // Doit correspondre a @version
