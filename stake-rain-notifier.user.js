...
// @version      1.5.130
// @description  StakePulse - Rain & Stats tracker pour Stake.bet - by alleluiateam | v1.5.130 - Ajout menu Wager Goals + Detecteur High Roller + Fix reset wager + Reset par periode + Session tracker + Migration wager goals + Fix Hot Words jour + Fix Emoji Stats jour + Filtre chats Hot Words + Fix Top Rainer jour + Fix Multi Tracker betId + notifs + Fix toggle + Fix seuil min 1.01
// @author       alleluiateam
...
'.srn-tick-down{color:#ff4444}',
  ...
  function refreshPanel() {
    ...
    // ── Meilleure rain aujourd'hui (depuis minuit) ─────────────────────────
    var todayMidnight = new Date(); todayMidnight.setHours(0,0,0,0);
    var myUserBestRain = (CONFIG.YOUR_USERNAME || '').toLowerCase().trim();
    var rains24h = logFiltered.filter(function(e) {
      if (e.ts < todayMidnight.getTime()) return false;
      if (!myUserBestRain) return true;
      if (!e.recipients || e.recipients.length === 0) return false;
      return e.recipients.some(function(r) { return r.toLowerCase() === myUserBestRain; });
    });
    var bestRain = rains24h.reduce(function(best, e) {
      var amt = parseFloat(e.usdEach || e.eurEach || 0);
    ...
    if (bestCard && bestEl) {
      if (bestRain > 0) {
        ...
        bestEl.textContent = bestRain.toFixed(2) + (logFiltered[0] && logFiltered[0].eurEach ? '\u20AC' : '$');
      } else {
  ...
  var CURRENT_VERSION = '1.5.130'; // Doit correspondre a @version
