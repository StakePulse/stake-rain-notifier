...
'.srn-tick-down{color:#ff4444}',
  ...
  var CURRENT_VERSION = '1.5.37'; // Doit correspondre a @version
  ...
  function renderMultip() {
    ...
    el.innerHTML = [
      ...
      '<div style="display:flex;flex-direction:column;gap:5px;margin-bottom:8px">',
        '<select id="srn-multip-provider" style="background:#162330;border:1px solid #1e3a4a;border-radius:6px;color:#fff;padding:4px 8px;font-size:11px"><option value="">-- Provider --</option>' + (bhProviders.length ? bhProviders.map(function(p) { return '<option value="' + escHtml(p.id) + '">' + escHtml(p.name) + '</option>'; }).join('') : '<option value="" disabled>Chargement...</option>') + '</select>',
        '<select id="srn-multip-game-name" style="background:#162330;border:1px solid #1e3a4a;border-radius:6px;color:#fff;padding:4px 8px;font-size:11px"><option value="">-- Choisir un jeu --</option></select>',
        '<div style="display:flex;gap:5px">',
    ...
    // Provider → filtre les jeux
    var provSel = document.getElementById('srn-multip-provider');
    var gameSel = document.getElementById('srn-multip-game-name');
    // Restaurer la sélection précédente
    var savedProv = load('srn_multip_last_prov', '');
    if (provSel && savedProv) {
      provSel.value = savedProv;
      // Recharger les slots du provider sauvegardé
      if (savedProv && !gameSel.options.length || gameSel.options.length <= 1) {
        bhLoadProviderSlots(savedProv, function(slots) {
          gameSel.innerHTML = '<option value="">-- Choisir un jeu --</option>';
          ...
          var savedGame = load('srn_multip_last_game', '');
          if (savedGame) gameSel.value = savedGame;
        });
      }
    }
    if (provSel && gameSel && !provSel._hasListener) {
      provSel._hasListener = true;
      provSel.addEventListener('change', function() {
        var provId = provSel.value;
        save('srn_multip_last_prov', provId);
        save('srn_multip_last_game', '');
        gameSel.innerHTML = '<option value="">Chargement...</option>';
        if (!provId) { gameSel.innerHTML = '<option value="">-- Choisir un jeu --</option>'; return; }
        bhLoadProviderSlots(provId, function(slots) {
          gameSel.innerHTML = '<option value="">-- Choisir un jeu --</option>';
          slots.forEach(function(s) {
            var opt = document.createElement('option');
            opt.value = s.name;
    ...
    if (gameSel && !gameSel._hasListener) {
      gameSel._hasListener = true;
      gameSel.addEventListener('change', function() {
        save('srn_multip_last_game', gameSel.value);
      });
    }
  }
