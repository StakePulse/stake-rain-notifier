...
// @version      1.5.116
// @description  StakePulse - Rain & Stats tracker pour Stake.bet - by alleluiateam | v1.5.116 - Ajout menu Wager Goals + Detecteur High Roller + Fix reset wager + Reset par periode + Session tracker + Migration wager goals + Fix Hot Words jour + Fix Emoji Stats jour + Filtre chats Hot Words + Fix Top Rainer jour + Fix Multi Tracker betId + notifs + Fix toggle + Fix seuil min 1.01
// @author       alleluiateam
...
'.srn-tick-down{color:#ff4444}',
  ...
  var CURRENT_VERSION = '1.5.116'; // Doit correspondre a @version
  ...
  function renderBetLog() {
    ...
    // Export CSV
    function exportCSV() {
      var rows = ['Date,Jeu,Multiplicateur,Mise,Payout,Profit,Devise,BetID'];
      filtered.forEach(function(w) {
        var d = new Date(w.ts).toLocaleString('fr-FR');
        var mult = (w.amount > 0 && w.payout > 0) ? (w.payout / w.amount).toFixed(2) : '';
        var betId = w.betId ? 'casino:' + w.betId : '';
        rows.push([
          '"' + d + '"',
          '"' + (w.game || 'Inconnu') + '"',
          mult,
          (w.amountFiat || 0).toFixed(4),
          (w.payoutFiat  || 0).toFixed(4),
          (w.profitFiat  || 0).toFixed(4),
          w.fiat || 'EUR',
          betId
        ].join(','));
      });
      var csv = rows.join('\n');
      var filename = 'stakepulse_bets_' + new Date().toISOString().slice(0,10) + '.csv';
      // GM_download disponible dans Tampermonkey — contourne la CSP de Stake
      if (typeof GM_download !== 'undefined') {
        GM_download({ url: 'data:text/csv;charset=utf-8,' + encodeURIComponent(csv), name: filename });
      } else {
        // Fallback data URI
    ...
    window._srnExportBetLog = exportCSV;
    ...
    el.innerHTML = [
      ...
      '<div style="display:flex;gap:4px;margin-bottom:8px;flex-wrap:wrap">',
        ...
        '</select>',
        '<button onclick="window._srnExportBetLog()" style="background:#00d4ff11;border:1px solid #00d4ff33;border-radius:6px;color:#00d4ff;padding:4px 8px;cursor:pointer;font-size:10px;font-weight:700;white-space:nowrap">\uD83D\uDCE5 CSV</button>',
      '</div>',
      // Résumé
